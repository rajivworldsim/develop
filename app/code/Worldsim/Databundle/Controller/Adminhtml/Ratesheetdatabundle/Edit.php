<?php
/**
 * Copyright © Worldsim_Databundle All rights reserved.
 * See COPYING.txt for license details.
 */
declare(strict_types=1);

namespace Worldsim\Databundle\Controller\Adminhtml\Ratesheetdatabundle;

class Edit extends \Worldsim\Databundle\Controller\Adminhtml\Ratesheetdatabundle
{

    protected $resultPageFactory;

    /**
     * @param \Magento\Backend\App\Action\Context $context
     * @param \Magento\Framework\Registry $coreRegistry
     * @param \Magento\Framework\View\Result\PageFactory $resultPageFactory
     */
    public function __construct(
        \Magento\Backend\App\Action\Context $context,
        \Magento\Framework\Registry $coreRegistry,
        \Magento\Framework\View\Result\PageFactory $resultPageFactory
    ) {
        $this->resultPageFactory = $resultPageFactory;
        parent::__construct($context, $coreRegistry);
    }

    /**
     * Edit action
     *
     * @return \Magento\Framework\Controller\ResultInterface
     */
    public function execute()
    {
        // 1. Get ID and create model
        $id = $this->getRequest()->getParam('id');
        $model = $this->_objectManager->create(\Worldsim\Databundle\Model\RateSheetDataBundle::class);
        
        // 2. Initial checking
        if ($id) {
            $model->load($id);
            if (!$model->getId()) {
                $this->messageManager->addErrorMessage(__('This Rate Sheet Data Bundle no longer exists.'));
                /** @var \Magento\Backend\Model\View\Result\Redirect $resultRedirect */
                $resultRedirect = $this->resultRedirectFactory->create();
                return $resultRedirect->setPath('*/*/');
            }
        }
        $this->_coreRegistry->register('worldsim_databundle_rate_sheet_data_bundle', $model);
        
        // 3. Build edit form
        /** @var \Magento\Backend\Model\View\Result\Page $resultPage */
        $resultPage = $this->resultPageFactory->create();
        $this->initPage($resultPage)->addBreadcrumb(
            $id ? __('Edit Rate Sheet Data Bundle') : __('New Rate Sheet Data Bundle'),
            $id ? __('Edit Rate Sheet Data Bundle') : __('New Rate Sheet Data Bundle')
        );
        $resultPage->getConfig()->getTitle()->prepend(__('Rate Sheet Data Bundles'));
        $resultPage->getConfig()->getTitle()->prepend($model->getId() ? __('Edit Rate Sheet Data Bundle %1', $model->getId()) : __('New Rate Sheet Data Bundle'));
        return $resultPage;
    }
}

