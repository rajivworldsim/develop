...
namespace Agtech\OrderAPI\Observer\Sales;
...
class OrderInvoicePay implements \Magento\Framework\Event\ObserverInterface
{
    ...
    public function __construct(
		...
		\Agtech\OrderAPI\Helper\Data $orderApiHelper,
	...
	public function execute(
		...
		try{
			...
			$orderid =  $orderIds[0];
			if($orderid){
				$this->pushOrderXml($orderid);	
			}
			return true;
		} catch (\Exception $e) {
			$this->sendEmailAlert($orderid, $e->getMessage());
			return true;
		}
        
	}
	
	public function sendEmailAlert($orderid, $errorMsg){
		$order = $this->_order->get($orderid);
		$lastOrderId = $order->getIncrementId();
		$this->orderApiHelper->sendEmail($orderid, $lastOrderId, $errorMsg);
	}
	...
	public function pushOrderXml($orderid) {
        $crm_orderNumb='';
...
//        }
        ...
        if($order->getShippingAddress()){
            ...
            $shippingAddress_firstname = str_replace("&","and",$shippingAddress_firstname);
            $shippingAddress_lastname = str_replace("&","and",$shippingAddress_lastname);
		...
		//if($_SERVER['HTTP_CF_IPCOUNTRY']=='PK'  || $billingAddress_countryId =='PK' || $shippingAddress_countryId == 'PK'){
		//	$VPSTxId = '';
		//	$sender = 'shahzad@worldsim.com';
		//	$recipient = 'shahzad.cs@gmail.com';
		//	
        ...
        $db_code =  file_get_contents($file_name_with_path);
                        ...
                        if(strpos(strtolower($_sku_id),"data-bundle-topup-new")!==false) { //NEW Data Bundle TopUp 2017
							...
							if($bundle_supplier == 'Go'){
								...
								if($bundle_simType == 'eSIM'){
									...
									$crm_numb_type = "DA";
									$esim_type_purchased = 'Internationl SIM Card';
								}
							}
                                ...
                                if($bundle_supplier == 'Go'){
                                    ...
                                    if($bundle_simType == 'eSIM' || $bundle_simType == 'Multiple SIM'){
                                        $crm_numb_type = 'WorldSIM-Data-eSIM';
										$esim_type_purchased = 'WorldSIM International eSIM Pro';
                                    }else{
                                        $crm_numb_type = "DA";
                                    }
                                }
       ...
       //send XML to CRM
        ...
        $item = new \stdClass();
        //$item->MagentoOrder = $xml_content;
        $item->MagentoOrder = $xml_content;
        
        $crm_orderResult = $client->CreateOrderInCRM($item);
        $crm_ord_numb = $crm_orderResult->CreateOrderInCRMResult;
        
        
        $item_get =  new \stdClass();
        $item_get->MagnentoOrderNo = $lastOrderId;
        $crm_orderResult_get = $client->GetOrderNumber($item_get);
        $crm_ord_numb_get = $crm_orderResult_get->GetOrderNumberResult;
        
        //$this->logger->info($xml_content);
        //$this->logger->info($crm_ord_numb_get);
        $order->setErpid($crm_ord_numb_get);
        $order->save();
