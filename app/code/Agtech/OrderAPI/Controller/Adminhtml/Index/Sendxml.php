...
class Sendxml implements HttpGetActionInterface
{
    ...
    {       
        ...
        if(strpos($paymentMethod,"sagepay")!==false){
            ...
            $CAVV = (array_key_exists('CAVV', $sagePayInfo)) ? $sagePayInfo['CAVV'] : '';
            $MSISDN = '';
            $CustomerID = '';
		...
		if($paymentPending){
			...
			echo "Order payment is pending, please check payment first.";
            exit;
        }
                ...
                if(is_array($_item->getProductOptions()) && array_key_exists('options',$_item->getProductOptions())){
                    ...
                    foreach ($_options as $option) {
                        ...
                        elseif(strpos(strtolower($option['label']),"credit")!==false){
                            ...
                            if(strpos($_sku_id,"top-up-")!==false || strpos($_item->getSku(),"upgraded")!==false){ 
                                if(strpos($_sku_id,"-auto")!==false){
                                    $_sku_id = str_replace('-auto', '', $_sku_id);
                                }
                                $creditCurrency = substr($_sku_id, -3);
                                $_creditCurrencyCode = strtoupper($creditCurrency);
                                foreach($rates[$baseCurrencyCode] as $key=>$value  ) {
        ...
        $order->save();
        if(strpos($crm_ord_numb_get,'ORD-') !== false):
            echo "Order Imported into CRM Succesfully, CRM Order Number => ".$crm_ord_numb_get;
        else: 
             echo "Order NOT impoted to CRM, Error => ".$crm_ord_numb; 
        endif;
