...
class GoogleFonts implements \Magento\Framework\Option\ArrayInterface
{
	...
	public function toOptionArray()
	{
		...
		array_unshift($options,['label'=>'--Select--','value'=>'']);
		return $options;
	}

	public function getGooglesFonts()
    {
		$names = explode(',', $this->_names);
		return $names;
	}
	...
	public function getOptions()
	{
		...
		$options = array();
		foreach ($names as $n){
			$options[] = array( 'value' => $n, 'label' => $n, 'class' => 'googlefont');
		}
		return $options;
	}

	public function isGoogleFont($fontName)
    {
		$names = explode(',', $this->_names);
		if(in_array($fontName, $names)){
			return true;
		}
		return false;
	}
