...
class Data extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    public function getCategoryById($categoryId)
    {
        if (!isset($this->loadedCategories[$categoryId])) {
            $this->loadedCategories[$categoryId] = $this->categoryRepository->get($categoryId, $this->_storeManager->getStore()->getId());
        }
     ...
     * @return string
     */
	public function drawItem($item, $level = 0, $x = 0, $listTag = true, $parentIndex = 1,$disable_mobile = true)
    {
		$hasChildren = false;
		...
		if ($item['link_type'] == 'custom_link') {
			...
			if (!empty($href) && ((strpos($href, ":javascript:") !== false) || (strpos($href, ":JAVASCRIPT:") !== false))) {
				$href = '#';
				...
				if ($item['show_content'] && ((($item['content_type'] == 'childmenu' || $item['content_type'] == 'dynamic') && $hasChildren) || $item['content_type'] == 'parentcat' || ($item['content_type'] == 'content' && $item['content_html']!=''))) {
					...
					if ($item['content_type'] == 'childmenu' && $hasChildren) {
						...
						foreach ($children as  $z => $it) {
							...
							$resultTmp[$x1][$levelTmp] = $this->drawItem($it, $level, $i, false, $parentChildIndex,$disable_mobile);
							$childIndex++;
					...
					if ($item['content_type'] == 'parentcat') {
						...
						foreach ($catChildren as $it) {
							...
							$html .= $this->drawItem($it, $level, $i, false, $parentChildIndex,$disable_mobile);
							$html .= '</div>';
							$childIndex++;
							if( $column == 1 || ($i+1)%$column == 0 || $i == ($total-1) ) {
								$html .= '</div>';
    ...
    /**
     * Endswidth search forward starting from end minus needle length characters
     *
     * @param string $haystrack
     * @param string $needle
     * @return bool|int
     */
	...
	public function getCategory($catId, $cat = '')
    {
		...
		if ($cat) {
            ...
            $url = $this->getCategoryUrlById((int)$catId);
