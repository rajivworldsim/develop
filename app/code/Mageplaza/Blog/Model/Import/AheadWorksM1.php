...
class AheadWorksM1 extends AbstractImport
{
    ...
    protected function _importPosts($data, $connection)
    {
        ...
        $oldPostIds   = [];
        $tags         = [];
        $data['database'] = "m1w0rkds3m_wdsim1";
            ...
            foreach ($sourceItems as $post) {
                ...
                if (!empty($postTag)) {
                    ...
                    foreach ($tagNames as $name) {
                        ...
                        $tagTableSql = "SELECT * FROM `" . self::TABLE_TAG . "` WHERE `tag` = '" . $name . "'";
    ...
    {
        $sqlString = "SELECT * FROM `" .self::TABLE_TAG . "`";
        $result    = mysqli_query($connection, $sqlString);
        $this->_resetRecords();
        $isReplace = true;
        $oldTagIds = [];
    ...
    {
        $sqlString = "SELECT * FROM `" . self::TABLE_CATEGORY . "`";
        $result    = mysqli_query($connection, $sqlString);
        $isReplace = true;
    ...
    protected function _importComments($data, $connection)
    {
        ...
        $sqlString     = "SELECT * FROM `" . self::TABLE_COMMENT . "`";
        $result        = mysqli_query($connection, $sqlString);
        $this->_resetRecords();
        $isReplace = true;
