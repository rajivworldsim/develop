...
<?php if($_POST['page'] == "Tariff-Widget-From") {
    ...
    foreach ($jsonData as $operatorRates){
        if($operatorRates->getcountry()==$fromCountry){
            $countryOperator = $jsonFactory->create()->addFieldToFilter('country',$fromCountry)->addFieldToFilter('outgoing',array('gt' => 0))->addFieldToFilter('operator',array('notnull' => true))->setOrder('data','ASC');
            $operators_options = '';
            $operator_counter = 0;
            foreach($countryOperator as $specificOperator){ ?>
                <?php
                    ...
                    if($operator_counter==0){ //show by default rate for fist operator
                        ...
                        $json_data['profile'] = $profile_from;
                    }
                    ?>
                    <?php $operators_options = $operators_options."<option value='".$specificOperator->getOperator()."' data-profile='".$profile_from."' data-datarate='".$data_receive."' data-incoming='".$in_cheap."' data-outgoing='".$out_cheap."' data-sms='".$sms_send."'>".$specificOperator->getOperator()."</option>"; ?>
